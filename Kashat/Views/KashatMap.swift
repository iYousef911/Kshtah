...
struct KashatMap: View {
        ...
        VStack(spacing: 0) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ...
                    // NEW: Heatmap Toggle Chip
                    FilterChip(
                        title: "AI زحام" + (store.userProfile?.isPro == true ? "" : " 🔒"),
                        icon: "person.3.fill",
                        isSelected: showHeatmap
                    ) {
                        if store.userProfile?.isPro == true {
                            withAnimation { showHeatmap.toggle() }
                        } else {
    ...
    @State private var showHeatmap = false // NEW: Heatmap Toggle
    ...
    private var mapContent: some MapContent {
        ...
        if showHeatmap {
            ForEach(filteredSpots) { spot in
                let prediction = CrowdService.shared.predictCrowd(for: spot, weather: currentWeather)
                // Use explicit colors to ensure visibility
                let color: Color = prediction.score >= 70 ? .red : (prediction.score >= 40 ? .orange : .green)
                
                MapCircle(center: spot.coordinate, radius: 4000)
                    .foregroundStyle(color.opacity(0.8))
                    .mapOverlayLevel(level: .aboveLabels) // Force on top of satellite imagery
            }
        }

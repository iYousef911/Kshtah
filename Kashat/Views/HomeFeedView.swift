...
struct HomeFeedView: View {
    ...
    var body: some View {
        NavigationStack { // NEW: Required for NavigationLinks
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    // Header
                    HStack {
                        VStack(alignment: .leading) {
                            // FIX: Use dynamic userName here
                            HStack {
                            ...
                            ForEach(categories, id: \.self) { category in
                                Button(action: {
                                    ...
                                    CategoryGlassPill(
                                        ...
                                        activeColor: store.appColor // NEW: Dynamic Color
                                    )
                                }
                                .buttonStyle(.plain)
                            }
                    ...
                    if store.showDiscountBanner {
                        HStack {
                            ...
                            .foregroundStyle(store.appColor) // Dynamic Color
                            .clipShape(Capsule())
                        }
                        .padding()
                        .background(
                    ...
                    if !filteredSpots.isEmpty {
                        ...
                        ForEach(filteredSpots) { spot in
                            GlassSpotCard(spot: spot)
                                ...
                                .onTapGesture {
                                    selectedSpot = spot // NEW: Trigger sheet
                                }
                        }
                    } else {
                        VStack(spacing: 12) {
            ...
            .toolbar(.hidden, for: .navigationBar) // Hide system nav bar to keep custom header
            .refreshable {
                store.fetchSpots()
            }
            .sheet(isPresented: $showInbox) { InboxView().presentationDetents([.large]) }
            .sheet(isPresented: $showNotifications) { NotificationsView().presentationDetents([.medium]) } // NEW
            .fullScreenCover(isPresented: $showCompass) { ARQiblaView() }
            // NEW: Spot Detail Sheet
